...
- **원격 접속 안 됨 (Tailscale·LAN)**: Ollama가 `127.0.0.1:11434`에만 바인딩된 경우. 보통 Homebrew가 `brew services start ollama`로 자체 launchd (`homebrew.mxcl.ollama.plist`)를 띄워 기본값으로 실행한 결과. 해결:
  1. `brew services stop ollama` — Homebrew 자동 시작 중단
  2. `brew services disable ollama` — 재부팅 후에도 자동 시작 방지 (영구, 권장)
  3. `./start.sh <model> --install` — 이 템플릿의 launchd 등록 (`OLLAMA_HOST=0.0.0.0:11434`, `OLLAMA_USE_MLX=1`)
  4. `lsof -nP -iTCP:11434 -sTCP:LISTEN` 로 `*:11434` 바인딩 확인
- **원격 접속 (Tailscale)**: MagicDNS로 hostname 접근 가능. 예: `http://<tailscale-hostname>.tailXXXXXX.ts.net:11434/v1`. Ollama는 인증 없음 — tailnet 내부 노드 전부 접근 가능하니 필요시 Tailscale ACL로 제한.
